...
def predictTumourImg(top, tumour_presence_str, predicted_output, image_name):
    ...
    if (DetectBrainTumour.predictTumourPresence(classifier, image_name.get())):
        ...
        tk.Label(top, textvariable=tumour_presence_str, fg='blue', bg='yellow', height=2, width=25).place(relx=0.41, rely=0.8)
    else:
...
def createWindow(master):
    ...
    tk.Button(top, text='Generate model', command=lambda: generateModel(top), width=15,height=2).place(relx=0.44, rely=0.04)
    tk.Button(top, text='getAccuracy_of_Type_Module',  command=lambda: getAccuracy(top, accuracy) , width=25, height=2).place(relx=0.25, rely=0.2)
    # classifier= B.invoke()
    tk.Button(top, text='getAccuracy_of_Tumour_Detect_Module', command=lambda: getAccuracyTumour(top, accuracyTumour), width=25,height=2).place(relx=0.55, rely=0.2)
    tk.Button(top, text='select_Img', command=lambda: selectImg(top, image_name), width=15,  height=2).place(relx=0.2, rely=0.4)
    tk.Button(top, text='predict_Img',  command=lambda: predictTumourImg(top, tumour_presence_str, predicted_output, image_name), width=15, height=2).place(relx=0.7, rely=0.4)
    tk.Button(top, text='plot_graph_mri', command=lambda: plotGraph(top), width=15, height=2).place(relx=0.3, rely=0.7)
    tk.Button(top, text='plot_roc', command=lambda: plot_roc_brain_tumour(top), width=15, height=2).place(relx=0.59, rely=0.7)
    top.mainloop()
